...
socket.on("availableSplatFilenames", (data) => {
  availableSplatFilenames = data;
  console.log({ availableSplatFilenames });
...
s.addEventListener("change", (ev) => {
  // console.log(ev);
  // console.log(s.value);
  selectFileFromServer(s.value);
});
const selectFileFromServer = async (filename) => {
  // Use Fetch API to make a request to the server
  const response = await fetch(`/gaussian-splats/${filename}`);
  ...
  // Parse the response as text or binary data depending on your PLY file format
  // const plyData = await response.text(); // Use response.arrayBuffer() for binary data

  // Process the PLY data as needed
  // console.log(plyData);
  ...
  splatDataAB = await response.arrayBuffer();

  splatData = new Uint8Array(splatDataAB);
...
function updateSelect() {
  let s = document.getElementById("filenameSelect");
  s.innerHTML = "";
  for (let i = 0; i < availableSplatFilenames.length; i++) {
    // Loop through the list and add options to the select element
    var option = document.createElement("option");
    option.text = availableSplatFilenames[i];
    // Optionally, you can set the value attribute as well:
    // option.value = stringList[i];
    s.add(option);
  }
}
let cameras = [
